# This step is to verify that some *screen* or *box* or *popup* or *border* is *displayed* or not.
Then /^'(.*)' (.+) should( not)? be displayed(?: in (\d+) seconds?)?( .+)?$/ do |object_name, object_type, shouldnot, second, scope|
  ...
  if container_class.new(object_name, container_locator).is_present?
    begin
      container_class.new(object_name, container_locator).wait(second).should == shouldnot.nil?
    rescue RSpec::Expectations::ExpectationNotMetError
      raise "#{object_type} with name '#{object_name}' expected#{shouldnot.nil? ? '' : ' not'} to be present but it doesn't."
    end
  else
