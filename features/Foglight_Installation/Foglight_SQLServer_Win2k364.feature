...
#
  ...
  @yammy @fms
  Scenario: Install Foglight on windows64bit with external SQLServer
    Given I have downloaded Foglight installer by the following parameters
      | credential | fogflsw02                                                                                             |
      | from       | /nightly/FoglightServerDist/trunk/*build_number*/windows-x86_64/Foglight-5_6_9_999-windows-x86_64.zip |
      | to         | C:/Foglight-5_6_9_999-windows-x86_64.zip                                                              |
    And I unzip 'C:/Foglight-5_6_9_999-windows-x86_64.zip' to '#product_installation_path#'
    ...
    And I copy file by the following parameters
      | from file    | *work_dir*/resources/sqlserver_server.config           |
      | from machine | localhost                                              |
      | to file      | #product_installation_path#/state/config/server.config |
      | to machine   | #host#                                                 |
    And I start process '#product_bin_path#/createdb.bat sa Rdis2fun' to create database
    Then I should see 'Populate for database "yammyc"' in db start command line in 300 seconds
