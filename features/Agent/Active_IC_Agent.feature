Feature: Active IC agent
  ...
  Scenario: Active IC agent
    ...
    And I receive result from command '#product_bin_path#/fglcmd.bat -usr foglight -pwd foglight -cmd agent:list -host #agent_host# -active true -type WindowsAgent -name Windows_IC_Agent' to get all active agent on win2k3-32 fglam
